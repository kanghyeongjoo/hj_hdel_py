...
def get_table_cdnt():
    ...
    if table_blo_y_cdnt != None:
        ...
        for entity in doc.ModelSpace:
            if entity.EntityName == 'AcDbText' and entity.TextString == "층" and entity.TextAlignmentPoint[
                1] == text_base_y_cdnt:
        ...
        for entity in doc.ModelSpace:  # 테이블 좌표 구하기
            if entity.EntityName == 'AcDbPolyline' and entity.Layer == "LAD-OUTLINE" and entity.Coordinates[
                0] == table_x_cdnt and entity.Coordinates[1] == table_y_cdnt:
                start_x_cdnt = entity.Coordinates[0]
                end_x_cdnt = entity.Coordinates[2]
                start_y_cdnt = entity.Coordinates[1]
                end_y_cdnt = entity.Coordinates[5]
...
def get_fl_and_fl_h(s_x_cdnt, e_x_cdnt, s_y_cdnt, e_y_cdnt):
    ...
    floors_data_with_x_cdnt = sorted(
        floors_data_with_x_cdnt.items())  # 가로 좌표 기준 정렬(단, 층표기 분리시 세로 좌표가 동일하므로 세로 층표기를 정렬이후에 나누는 것도 고려해볼 것)
