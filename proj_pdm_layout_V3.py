...
def get_entity(layout_kind):
    ...
    if layout_kind == "E":
        ent_blo_name = ["LAD-TITLE", "LAD-DOOR-JAMB", "LAD-OPEN-HOLE", "LAD-HBTN", "LAD-OPEN-AC", "LAD-HALL-LANTERN", "LAD-EMCY-SWITCH",
                        "LAD-REMOTE-CP"]
        ent_group = dict.fromkeys(ent_blo_name)
        ent_and_cdnt = {}
        for entity in doc.ModelSpace:
...
def get_hall_data(floor_data):
    ...
    for tit_ent in ent_group["LAD-TITLE"]:
        ...
        if "소방스위치" in hall_items.keys():
            ...
            entrance_data.update({"EL_CFRSW":fireman_sw})
        ...
        if "기준층" in app_floor_info:
            split_floor_txt = re.split("층", app_floor_info)
            for floor_txt in split_floor_txt:
                if "기준" not in floor_txt and floor_txt.isalnum():
                    app_floor = re.findall("(\w+)층", app_floor_info)[0]
...
def get_hall_items(hall_odr, jamb_spec):
    ...
    floor_items = {"JAMB": jamb_type, "JAMB_ORD": app_jamb, "HH":hh, "JJ":int(jj_l + jj_r), "홀버튼": btn_spec, "홀버튼_취부": btn_type, "HPI": hpi}
...
def get_proj_data(proj_no):
    ...
    get_entity("E")
    df_floor, entrance_data = get_hall_data(df_floor)
...
proj_data, floor_data = get_proj_data("180944L01")
print(proj_data)
