...
def get_spec(ouid):
    ...
    for split_code in split_fl_list:
        if spec_get[split_code] != "":
            cvt_fl = ",".join(special_str_split(spec_get[split_code]))
            spec_get.update({split_code: cvt_fl})

    if spec_get["EL_ECN"].isdigit():
        spec_get.update({"EL_ECN": int(spec_get["EL_ECN"])})
        ...
        if "LAD-HOISTWAY-TP-WB" in ent_name:
            if ent_name[-2:] == "SC":
                ...
                for hoist_prt in m_ent.GetDynamicBlockProperties():
                    if hoist_prt.PropertyName == "@HA-L":
                        hoist_l = int(hoist_prt.value)
                        mrl_data.update({"EL_ECHOR": hoist_l})
                    elif hoist_prt.PropertyName == "@HA-R":
                        hoist_r = int(hoist_prt.value)
                    elif hoist_prt.PropertyName == "@OH":
                        mrl_data.update({"EL_EHO": str(hoist_prt.value)})
            ...
            for car_prt in m_ent.GetDynamicBlockProperties():
                if car_prt.PropertyName == "@CAR-H":
                    mrl_data.update({"EL_ECCH": str(car_prt.value)})
                elif car_prt.PropertyName == "@DOOR-H":
                    mrl_data.update({"EL_ECHH": str(car_prt.value)})
                elif car_prt.PropertyName == "@JJ-L":
                    jj_l = int(car_prt.value)
                elif car_prt.PropertyName == "@JJ-R":
                    jj_r = int(car_prt.value)
...
def get_hall_data(floor_data):
    ...
    for tit_ent in ent_group["LAD-TITLE"]:
        ...
        if "기준층" in app_floor_info:
            ...
            entrance_data.update({"EL_EMF": main_floor})
    ...
    for jamb_type, app_jamb in jamb_type_list.items():
        if jamb_type in list(df_jamb["JAMB"]):
            for idx in list(df_jamb.index[df_jamb["JAMB"] == jamb_type]):
                ...
                entrance_data.update({"EL_CJM" + str(jamb_ord) + "Q": len(jamb_app_floor_list)})
                entrance_data.update({"EL_CJM" + str(jamb_ord) + "F": jamp_app_flooor})
                jamb_ord = jamb_ord + 1
...
def get_proj_data(proj_no):
    ...
    df_in_layout_data = df_spec.drop(df_spec.columns[df_spec.iloc[1:].isna().all()], axis=1)

    Hbtn_code_list = ["EL_CHIPB", "EL_CHIPM1", "EL_CHIPM2", "EL_CHIPT", "EL_CHPBB", "EL_CHPBM1", "EL_CHPBM2",
                      "EL_CHPBT"]
    for Hbtn_code in Hbtn_code_list:
        if Hbtn_code in list(df_in_layout_data.columns):
            if pd.notnull(df_in_layout_data.iloc[0][Hbtn_code]):
                if "230" in df_in_layout_data.iloc[0][Hbtn_code]:
                    df_in_layout_data.loc["PDM_DATA", Hbtn_code] = "BOX"
                else:
                    df_in_layout_data.loc["PDM_DATA", Hbtn_code] = "BOXLESS"
    ...
    for Hpi_code, Jamb_code in Hpi_Jamb_code.items():
        if Hpi_code in list(df_in_layout_data.columns):
            if pd.notnull(df_in_layout_data.iloc[0][Hpi_code]):
                if df_in_layout_data.iloc[0][Jamb_code][2] == "2":
                    df_in_layout_data.loc["PDM_DATA", Hpi_code] = "JAMB 취부"
                else:
                    if "310" in df_in_layout_data.iloc[0][Hpi_code]:
                        df_in_layout_data.loc["PDM_DATA", Hpi_code] = "BOX"
                    else:
                        df_in_layout_data.loc["PDM_DATA", Hpi_code] = "BOXLESS"

    df_spec.fillna("", inplace=True)
    df_floor.fillna("", inplace=True)
    df_in_layout_data.fillna("", inplace=True)

    df_in_layout_data.loc["최종"] = ""

    for col in list(df_in_layout_data.columns):
        col_val = list(df_in_layout_data[col].unique())

        if "" in col_val:
            col_val.remove("")

        if len(col_val) == 1:  # 값이 유일하면
            df_in_layout_data.loc["최종", col] = col_val[0]
        else:
            df_in_layout_data.loc["최종", col] = "Check"

    return df_spec, df_in_layout_data, df_floor
...
def run_comparison():
    project_no = proj_no_ent.get()
    proj_total_data, plm_n_layout_data, floor_data = get_proj_data(project_no)

    df_col = list(proj_total_data.columns)
    df_col.insert(0, "특성코드")
    df_idx = list(proj_total_data.index)

    notebook = tk.ttk.Notebook(window, width=980, height=620)  # tap 나누기
    notebook.place(x=10, y=50)

    frame1 = tk.Frame(window)
    frame1.place(x=10, y=50, width=950, height=600)
    notebook.add(frame1, text="ALL DATA")

    frame2 = tk.Frame(window)
    frame2.place(x=10, y=50, width=950, height=600)
    notebook.add(frame2, text="MATCH DATA")

    frame3 = tk.Frame(window)
    frame3.place(x=10, y=50, width=950, height=600)
    notebook.add(frame3, text="DATA UPDATE")

    frame4 = tk.Frame(window)
    frame3.place(x=10, y=50, width=950, height=600)
    notebook.add(frame3, text="FLOOR TABLE")

    tv = ttk.Treeview(frame1, column=df_col, show='headings', height=5)
    tv.place(relheight=1, relwidth=1)

    v_scr_bar = tk.Scrollbar(frame1, orient="vertical", command=tv.yview)
    h_scr_bar = tk.Scrollbar(frame1, orient="horizontal", command=tv.xview)
    tv.configure(yscrollcommand=v_scr_bar.set, xscrollcommand=h_scr_bar.set)
    v_scr_bar.pack(side="right", fill="y")
    h_scr_bar.pack(side="bottom", fill="x")

    for col_v in df_col:
        tv.column(col_v, width=100, minwidth=50)
        tv.heading(col_v, text=col_v)

    for idx_no in range(len(df_idx)):
        idx_value = list(proj_total_data.loc[df_idx[idx_no]])
        idx_value.insert(0, df_idx[idx_no])
        tv.insert(parent='', index=idx_no, text=df_idx[idx_no], values=idx_value)



window = tk.Tk()
window.title("PDM & LAYOUT DATA")
window.geometry("1000x700+50+50")
window.pack_propagate(False)
window.resizable(0, 0)

proj_no_ent = tk.Entry(window)
proj_no_ent.place(x=10, y=10, width=100, height=25)

btn1 = tk.Button(window, text="비교 실행", relief="raised", overrelief="ridge", command=run_comparison)
btn1.place(x=120, y=10, width=80, height=25)

btn2 = tk.Button(window, text="Mach", relief="raised", overrelief="ridge", command=mach_data)
btn2.place(x=250, y=10, width=80, height=25)

btn3 = tk.Button(window, text="All", relief="raised", overrelief="ridge", command=all_data)
btn3.place(x=340, y=10, width=80, height=25)
window.mainloop()
