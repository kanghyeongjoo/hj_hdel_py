...
def get_floor_data():
    ...
    for entity in doc.ModelSpace:
        if entity.EntityName == 'AcDbBlockReference' and entity.EffectiveName == "LAD-FORM-A3-SIMPLE":
            ...
            firedoor_table_y_cdnt = entity.InsertionPoint[1]
            entity.Explode() # 방화도어 테이블 분해

    floor_table_cdnt = get_floor_table_cdnt(floor_table_y_cdnt, palette_area)
    floor_data, total_floor, stop_floor = get_floor_height(*floor_table_cdnt)

    return floor_data, total_floor, stop_floor


def get_floor_table_cdnt(floor_table_y_cdnt, palette_area):

    if floor_table_y_cdnt != None:
        for entity in doc.ModelSpace:  # 테이블 좌표 구하기
            if entity.EntityName == 'AcDbPolyline' and entity.Coordinates[1] == floor_table_y_cdnt:
